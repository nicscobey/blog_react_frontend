        ...
        const smallCards = getLatestPosts().map((post) => {
            return (
                <div key={`small-card-${post.id}`} className="blog-card-small">
                    <Link to={`/${target}/${post.id}`}><img className="card-image-small" src={post.banner !== "" ? post.banner : "https://e7.pngegg.com/pngimages/653/502/png-clipart-gray-draft-text-on-black-background-postage-stamps-draft-miscellaneous-angle.png"} alt=""/></Link>
